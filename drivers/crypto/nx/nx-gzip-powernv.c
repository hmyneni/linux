...
static int nx_ioc_gzip_tx_win_open(struct file *fp, unsigned long arg)
{
	...
	/*
	 * A send window for each open call.
	 */
	if (nxti->txwin)
		return -EEXIST;
...
static int nxgzip_release(struct inode *inode, struct file *fp)
{
	...
	/*
	 * We don't know here if user has other receive windows open,
	 * so we can't really call clear_thread_tidr(). so once the
	 * process calls set_thread_tidr(), the TIDR value sticks
	 * around until process exits, resulting in an extra copy in
	 * restore_sprs().
	 */
...
int nxgzip_device_create(void)
{
	...
	if (rc) {
		pr_err("NX-GZIP: cdev_add() failed %d\n", rc);
		goto err_cdev;
	}
